...
norin_brm2 |> hypothesis('TRIALHypoxia=0') |> plot()
norin_brm2 |> hypothesis('scaleSMR_contr=0') |> plot()
```

```{r posterior2h2a, results='markdown', eval=TRUE, fig.width = 7, fig.height = 5}
norin_brm2 |> SUYR_prior_and_posterior()
```
...
norin_brm3 |> mcmc_plot(type='trace')
```
...
norin_brm3 |> mcmc_plot(type='acf_bar')
```
...
norin_brm3 |> mcmc_plot(type='rhat_hist')
```
...
norin_brm3 |> mcmc_plot(type='neff_hist')
```
...
norin_brm3 |> mcmc_plot(type='combo')
norin_brm3 |> mcmc_plot(type='violin')
```
</details>
...
norin_ggs <- norin_brm3 |> ggs(burnin = FALSE, inc_warmup = FALSE)
...
ggs_effective(norin_ggs, version_effective = "BDA3")
```
...
norin_brm3 |> pp_check(type = 'dens_overlay', ndraws = 100)
```
...
## norin_brm3 |> pp_check(type = 'error_scatter_avg')
```
...
norin_brm3 |> pp_check(group = 'TRIAL', type = 'intervals')
norin_brm2 |> pp_check(group = 'TRIAL', type = 'intervals_grouped')
norin_brm2 |> pp_check(group = 'TRIAL', type = 'violin_grouped')
```
...
norin_resids <- make_brms_dharma_res(norin_brm3, integerResponse = FALSE)
...
- the intercept indicates that the average metabolic change associated with the
  ...
  fish was `r round(norin_sum$fixed[1,1], 2)`.
- for the same SMR_contr and MASS, the average metabolic change was
  `r round(-1*norin_sum$fixed[2,1], 2)` less in the Hypoxia trial and
  `r round(-1*norin_sum$fixed[3,1], 2)` less in the Salinity trial than
  the High Temperature trial.
- in the High Temperature trial, every 1 unit increase in SMR_contr was
  associated with a decline in metabolic change of `round(-1*norin_sum$fixed[4,1], 2)`
  units.
...
norin_tidy <- tidyMCMC(norin_brm3$fit, estimate.method='median',
                         conf.int=TRUE,  conf.method='HPDinterval',
                         rhat=TRUE, ess=TRUE)
```
...
norin_brm3$fit |> plot(type='intervals')
```
...
0.1 * sd(norin$CHANGE)
norin_brm3 |> rope(range = c(-3.38, 3.38))
rope(norin_brm3, range = c(-3.38, 3.38)) |> plot()
...
#| echo: true
#| cache: false
norin_brm3 |> modelplot(exponentiate = FALSE)
```
::::
:::
<!-- END_PRIVATE-->
...
```{r posteriors1a, results='markdown', eval=TRUE, echo=TRUE, mhidden=TRUE}
norin_brm3 |> emtrends(~TRIAL, var='SMR_contr')
norin_brm3 |> emtrends(~TRIAL, var='SMR_contr') |> pairs()
norin_brm3 |> emtrends(~TRIAL, var='SMR_contr') |> pairs() |>
  tidy_draws() |>
  summarise_draws(median, HDInterval::hdi,
                  Pl = ~ mean(.x < 0),
                  Pg = ~ mean(.x > 0))
norin_emt <- norin_brm3 |> emtrends(~TRIAL, var='SMR_contr') |> pairs() |> as.data.frame()
```

**Conclusions:**

- the partial slope associated with control SMR during High
  Temperature was found to be `r -1*round(norin_emt[1, 2], 2)` units
...
- the partial slope associated with control SMR during High
  Temperature was found to be `r -1*round(norin_emt[2, 2], 2)` units
...
- the partial slope associated with control SMR during Hypoxia was
  found to be `r round(norin_emt[3, 2], 2)` units less negative than
...
norin_grid
norin_brm3 |> emmeans(~TRIAL|SMR_contr,  at=norin_grid) |> pairs()
...
newdata <- norin_brm3 |> emmeans(~SMR_contr|TRIAL, at = norin_grid) |>
    as.data.frame()
head(newdata)
