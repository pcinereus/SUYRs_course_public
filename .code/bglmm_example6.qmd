...
Area of the plate segment was also incorporated as an offset.
...
Whilst, the data only include fanworm densities (thereby potentially
limiting our choice of model family to Gaussian, log-normal, gamma and
tweedie), it turns out we also happen to know the area of each plate
segment (DIST).  Each plate segment is 4 units of area (note, that
does mean that the schematic above does not really represent the true
relative radius of each segment).

So, if we know the area of each segment, we can reconstruct the actual
fanworm counts (by multiplying the densities by the area) - great!
Doing so would allow us to use arguably more appropriate distributions
(like Poisson and Negative Binomial) and if we also incorporate area
as an offset, we can still express the parameters in density units.
...
- $\beta_0$: normal centred at 2.4 with a standard deviation of 0.2
  - mean of 2.4: since `median(log(copper$WORMS+0.1))` or `median(asinh(copper$WORMS/(2*1))/log(exp(1)))`
  - sd of 0.2: since `mad(log(copper$WORMS+0.1))` or `mad(asinh(copper$WORMS/(2*1))/log(exp(1)))`
- $\beta_{1-2}$: normal centred at 0 with a standard deviation of 0.9
  - sd of 0.9: since `mad(log(copper$WORMS+0.1))/model.matrix(~COPPER*DIST, data = copper) |> apply(2, sd) `
- $\beta_{3-5}$: normal centred at 0 with a standard deviation of 1
  - sd of 1: since `mad(log(copper$WORMS+0.1))/model.matrix(~COPPER*DIST, data = copper) |> apply(2, sd) `
- $\beta_{6-11}$: normal centred at 0 with a standard deviation of 1.5
  - sd of 1.5: since `mad(log(copper$WORMS+0.1))/model.matrix(~COPPER*DIST, data = copper) |> apply(2, sd) `
- $\sigma_j$: half-t with parameters 0 and 1.

```{r fitModel2h, results='markdown', eval=TRUE, mhidden=TRUE}
copper |>
  group_by(COPPER, DIST) |>
  summarise(
    median = median(log(WORMS + 0.1)),
    mad = mad(log(WORMS + 0.1)),
    sd = sd(log(WORMS + 0.1))
  )
```
...
```{r fitModel2h0, results='markdown', eval=TRUE, mhidden=TRUE}
priors <- prior(normal(2.4, 0.2), class = 'Intercept') +
    prior(normal(0, 5), class = 'b') +
    prior(student_t(3, 0, 1), class = 'sd')

copper_form <- bf(COUNT ~ offset(log(AREA)) + COPPER * DIST + (1|PLATE),
                family = poisson(link = "log"))
copper_brm1 <- brm(copper_form,
                 data = copper,
                 prior = priors,
                 sample_prior = 'only',
                 iter = 3000,
...
```{r partialPlot2h0a, results='markdown', warning=FALSE, message=FALSE, eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
# conditions should not be necessary since the offset is already in the model.
copper_brm1 |>
    conditional_effects("COPPER:DIST") |>
    plot(points = TRUE)
copper_brm1 |>
    conditional_effects("COPPER:DIST", conditions = list(AREA = 4)) |>
    plot(points = TRUE)
copper_brm1 |>
...
```{r fitModel2h0ab, results='markdown', warning=FALSE, eval=TRUE, mhidden=TRUE}
copper_brm2 <- update(copper_brm1,
                       sample_prior = 'yes',
                       control = list(adapt_delta = 0.99),
                       seed = 123,
                       refresh = 0)
...
```{r partialPlot2h00a, results='markdown', warning=FALSE, message=FALSE, eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
copper_brm2 |>
    conditional_effects("COPPER:DIST", conditions = list(AREA = 4)) |>
    plot(points = TRUE)
...
copper_form <- bf(I(WORMS + 0.1)~ COPPER * DIST + (1|PLATE),
                family=lognormal())
copper_brm3a <- brm(copper_form,
                 data = copper,
                 prior = priors,
                 sample_prior = 'only',
                 iter = 5000,
...
copper_brm3b <- update(copper_brm3a,
                       sample_prior = 'yes',
                       control = list(adapt_delta = 0.99),
                       seed = 123,
                       refresh = 0)
...
copper_form <- bf(I(WORMS + 0.1) ~ COPPER * DIST + ( DIST |PLATE),
                family=Gamma(link='log'))
copper_brm4a <- brm(copper_form,
                 data = copper,
                 prior = priors,
                 sample_prior = 'only',
                 iter = 5000,
...
copper_brm4b <- update(copper_brm4a,
                       sample_prior = 'yes',
                       control = list(adapt_delta = 0.99),
                       refresh = 0)
...
```{r posterior2h0, results='markdown', eval=TRUE}
copper_brm2 |> get_variables()
copper_brm2 |> hypothesis('COPPERWeek1=0') |> plot()
copper_brm2 |> hypothesis('DIST2=0') |> plot()
```

```{r posterior2h0a, results='markdown', eval=TRUE, fig.width=10, fig.height=10}
copper_brm2 |> SUYR_prior_and_posterior()
```
...
copper_brm3b |> get_variables()
copper_brm3b |> hypothesis('COPPERWeek1=0') |> plot()
copper_brm3b |> hypothesis('DIST2=0') |> plot()
```

```{r posterior2h2a, results='markdown', eval=TRUE, fig.width=10, fig.height=10}
copper_brm3b |> SUYR_prior_and_posterior()
```

#### Gamma

```{r posterior2i2, results='markdown', eval=TRUE}
copper_brm4b |> get_variables()
copper_brm4b |> hypothesis('COPPERWeek1=0') |> plot()
copper_brm4b |> hypothesis('DIST2=0') |> plot()
```

```{r posterior2i2a, results='markdown', eval=TRUE, fig.width=10, fig.height=10}
copper_brm4b |> SUYR_prior_and_posterior()
```
...
```{r modelValidation2b0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=10, fig.height=10}
pars <- copper_brm2 |> get_variables()
pars <- pars |> str_extract('^b.Intercept|^b_COPPER.*|^b_DIST.*|[sS]igma|^sd.*') |>
    na.omit()
pars
copper_brm2 |> mcmc_plot(type='trace', variable = pars)
##OR
copper_brm2 |> mcmc_plot(type='trace',
...
```{r modelValidation2c0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=10, fig.height=4}
copper_brm2 |> mcmc_plot(type='acf_bar', variable = pars)
##OR
copper_brm2 |> mcmc_plot(type='acf_bar',
                        variable = '^b.Intercept|^b_COPPER.*|^b_DIST.*|[sS]igma|^sd.*',
                        regex = TRUE)
```
...
```{r modelValidation2d0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
copper_brm2 |> mcmc_plot(type='rhat_hist')
```
...
```{r modelValidation2e0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
copper_brm2 |> mcmc_plot(type='neff_hist')
```

  Ratios all very high.

<details><summary>More diagnostics</summary>
```{r modelValidation2f0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
copper_brm2 |> mcmc_plot(type='combo', pars = pars)
copper_brm2 |> mcmc_plot(type='violin', pars = pars)
```
</details>
...
pars <- copper_brm3b |> get_variables()
pars <- pars |> str_extract('^b.Intercept|^b_COPPER.*|^b_DIST.*|[sS]igma|^sd.*') |>
    na.omit()
pars
copper_brm3b |> mcmc_plot(type='trace', variable = pars)
##OR
copper_brm3b |> mcmc_plot(type='trace',
                        variable = '^b.Intercept|^b_COPPER.*|^b_DIST.*|[sS]igma|^sd.*',
                        regex = TRUE)
...
copper_brm3b |> mcmc_plot(type='acf_bar', variable = pars)
##OR
copper_brm3b |> mcmc_plot(type='acf_bar',
                        variable = '^b.Intercept|^b_COPPER.*|^b_DIST.*|[sS]igma|^sd.*',
                        regex = TRUE)
```
...
copper_brm3b |> mcmc_plot(type='rhat_hist')
```
...
copper_brm3b |> mcmc_plot(type='neff_hist')
```
...
copper_brm3b |> mcmc_plot(type='combo', pars = pars)
copper_brm3b |> mcmc_plot(type='violin', pars = pars)
```
</details>
...
pars <- copper_brm4b |> get_variables()
pars <- pars |> str_extract('^b.Intercept|^b_COPPER.*|^b_DIST.*|[sS]igma|^sd.*') |>
    na.omit()
pars
copper_brm4b |> mcmc_plot(type='trace', variable = pars)
##OR
copper_brm4b |> mcmc_plot(type='trace',
                        variable = '^b.Intercept|^b_COPPER.*|^b_DIST.*|[sS]igma|^sd.*',
                        regex = TRUE)
...
copper_brm4b |> mcmc_plot(type='acf_bar', variable = pars)
##OR
copper_brm4b |> mcmc_plot(type='acf_bar',
                        variable = '^b.Intercept|^b_COPPER.*|^b_DIST.*|[sS]igma|^sd.*',
                        regex = TRUE)
```
...
copper_brm4b |> mcmc_plot(type='rhat_hist')
```
...
copper_brm4b |> mcmc_plot(type='neff_hist')
```
...
copper_brm4b |> mcmc_plot(type='combo', pars = pars)
copper_brm4b |> mcmc_plot(type='violin', pars = pars)
```
</details>
...
```{r modelValidation2g0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=10, fig.height=10}
copper_brm2$fit |> stan_trace()
copper_brm2 |> get_variables()
pars <- copper_brm2 |> get_variables()
pars <- str_extract(pars, "^b_.*|^sd.*") |> na.omit()

copper_brm2$fit |> stan_trace(pars = pars)
```
...
```{r modelValidation2h0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=10, fig.height=10}
copper_brm2$fit |>
    stan_ac(pars = pars)
```
...
```{r modelValidation2i0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
copper_brm2$fit |> stan_rhat()
```
...
```{r modelValidation2j0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
copper_brm2$fit |> stan_ess()
```

  Ratios all very high.

```{r modelValidation2k0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=10, fig.height=10}
copper_brm2$fit |>
...
## owls.ggs <- copper_brm3 |> ggs(burnin = FALSE, inc_warmup = FALSE)
...
```{r modelValidation5a0, results='markdown', eval=FALSE, mhidden=TRUE, fig.width=6, fig.height=4}
available_ppc()
```
...
```{r modelValidation5b0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
copper_brm2 |> pp_check(type = 'dens_overlay', ndraws = 100)
```
...
```{r modelValidation5c0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## copper_brm2 |> pp_check(type = 'error_scatter_avg')
```
...
```{r modelValidation5e0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
copper_brm2 |> pp_check(group = 'Nest', type = 'intervals')
```
...
```{r modelValidation5g0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
#library(shinystan)
#launch_shinystan(copper_brm1)
```
...
copper_brm3b |> pp_check(type = 'dens_overlay', ndraws = 100)
```
...
## copper_brm3b |> pp_check(type = 'error_scatter_avg')
```
...
copper_brm3b |> pp_check(group = 'Nest', type = 'intervals')
```
...
copper_brm4b |> pp_check(type = 'dens_overlay', ndraws = 100)
```
...
## copper_brm4b |> pp_check(type = 'error_scatter_avg')
```
...
copper_brm4b |> pp_check(group = 'Nest', type = 'intervals')
```
...
```{r modelValidation6a0, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=10, fig.height=10}
copper.resids <- make_brms_dharma_res(copper_brm2, integerResponse = TRUE)
...
```{r validation2g0, results='markdown', eval=TRUE, error=TRUE,mhidden=TRUE, fig.width=7, fig.height=5, cache=FALSE, message=FALSE, warning=FALSE}
...
## preds <- copper_brm3a |> posterior_predict(nsamples = 250,  summary = FALSE)
...
copper.resids <- make_brms_dharma_res(copper_brm3b, integerResponse = TRUE)
wrap_elements(~testUniformity(copper.resids)) +
               wrap_elements(~plotResiduals(copper.resids, form = factor(rep(1, nrow(copper))))) +
               wrap_elements(~plotResiduals(copper.resids, quantreg = TRUE)) +
               wrap_elements(~testDispersion(copper.resids))
...
## preds <- copper_brm3b |> posterior_predict(nsamples = 250,  summary = FALSE)
## copper.resids <- createDHARMa(simulatedResponse = t(preds),
##                             observedResponse = copper$NCalls,
##                             fittedPredictedResponse = apply(preds, 2, median),
##                             integerResponse = TRUE)
...
copper.resids <- make_brms_dharma_res(copper_brm4b, integerResponse = TRUE)
wrap_elements(~testUniformity(copper.resids)) +
               wrap_elements(~plotResiduals(copper.resids, form = factor(rep(1, nrow(copper))))) +
               wrap_elements(~plotResiduals(copper.resids, quantreg = TRUE)) +
               wrap_elements(~testDispersion(copper.resids))
...
copper.sum <- summary(copper_brm2)
```
...
copper_brm2 |>
  ...
  ## tt(digits = 1)
  ## tt() |> format_tt(digits = 2, num_fmt = "significant_cell")
  ## tt() |> format_tt(digits = 2, num_fmt = "decimal")
  copper_brm2 |>
      as_draws_df() |>
      dplyr::select(matches("^b_.*|^sd.*")) |>
      mutate(across(matches("^b_.*"), exp)) |>
...
copper.tidy <- tidyMCMC(copper_brm2$fit, estimate.method='median',
                         conf.int=TRUE,  conf.method='HPDinterval',
                         rhat=TRUE, ess=TRUE)
```
...
copper_brm2$fit |> plot(type='intervals')
```
...
#| echo: true
#| cache: false
copper_brm2 |> modelplot(exponentiate = TRUE)
```
::::
:::
<!-- END_PRIVATE-->
...
newdata <- copper_brm2 |>
    emmeans(~COPPER|DIST, type='response', offset = log(1)) |>
    as.data.frame()
head(newdata)
ggplot(newdata) +
...
```{r postHoc2, results='markdown', eval=TRUE,mhidden=TRUE}
copper_brm2 |>
...
copper_brm2 |>
    ...
    ggplot(aes(x = fit, y = reorder(contrast, fit))) +
    geom_vline(xintercept = 1, linetype = "dashed") +
    stat_slabinterval(
        aes(fill = COPPER, color = COPPER),
...
## But the interval is hard to read!
copper |>
  group_by(COPPER, DIST) |>
  summarise(median(WORMS), median(COUNT), mean(COUNT))
copper_brm2 |>
  emmeans(~ DIST | COPPER, type = "response") |>
  pairs(reverse = TRUE) |>
  gather_emmeans_draws() |>
  ...
  # Median and HDI, nudged slightly below the slab
  stat_pointinterval(
    aes(color = COPPER),
    point_interval = "median_hdci",
  ...
  scale_x_continuous("Effect size (% scale)",
    trans = scales::log2_trans(),
    labels = function(x) 100 * (x-1),
    breaks = c(0.5, 0.8, 1, 1.25, 1.5, 2, 3, 6, 11, 21, 51)) +
  scale_y_discrete("") +
