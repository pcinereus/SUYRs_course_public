...
OR alternatively, we could write this as
...
where $\beta_{0}$ is the y-intercept (mean of the first group),
$\beta_{[1,2,3]}$ are the vector of effects parameters (contrasting
each group mean to that of the first group and $\boldsymbol{\beta}$ is
a vector of effects parameters (including the intercept) and
$\bf{X_i}$ is a model matrix representing the intercept and treatment
contrasts for the effects of density and season on mussel recruitment.
...
Given that there are some zero's, we need to add a small constant before
logarithmic transform.  It might also necessary to use means/sd rather than
median/mad
...
- $\beta_0$: normal centred at 2.5 with a standard deviation of 0.2
...
quinn_brm2 <- quinn_brm1 |> update(sample_prior = 'yes', refresh = 0)
```

### Plotting prior and posterior

```{r fitModel2k1, results='markdown', eval=TRUE, mhidden=TRUE}
quinn_brm2 |> get_variables()
quinn_brm2 |> hypothesis('fSEASONSummer<0') |> plot()
quinn_brm2 |> hypothesis('DENSITYLow<0') |> plot()
quinn_brm2 |> hypothesis('fSEASONSummer:DENSITYLow<0') |> plot()
```

```{r fitModel2k2, results='markdown', out.width = 600, fig.width = 8, fig.height = 4, eval=TRUE, mhidden=TRUE}
quinn_brm2 |> SUYR_prior_and_posterior()
quinn_brm2 |>
  posterior_samples() |>
  dplyr::select(-`lp__`) |>
...
ggs_effective(quinn_ggs, version_effective = "BDA3")
```
...
quinn_brm2$fit |> stan_trace()
quinn_brm2$fit |> stan_trace(inc_warmup=TRUE)
```
...
quinn_brm2 |> pp_check(type = 'dens_overlay', ndraws = 100)
```
...
#quinn_brm2 |> pp_check(type = 'error_scatter_avg')
```
...
quinn_brm2 |> pp_check(type='intervals')
## quinn_brm2 |> pp_check(group='DENSITY', type='intervals')
```
...
quinn_resids <- make_brms_dharma_res(quinn_brm2, integerResponse = TRUE)
...
preds <- quinn_brm2 |> posterior_predict(nsamples = 250,  summary = FALSE)
quinn_resids <- createDHARMa(simulatedResponse = t(preds),
                            observedResponse = quinn$RECRUITS,
                            fittedPredictedResponse = apply(preds, 2, median),
                            integerResponse = TRUE)
...
quinn_resids <- make_brms_dharma_res(quinn_brm3, integerResponse = TRUE)
wrap_elements(~testUniformity(quinn_resids)) +
               ## wrap_elements(~plotResiduals(quinn_resids, form = factor(rep(1, nrow(quinn))))) +
               wrap_elements(~plotResiduals(quinn_resids, quantreg = TRUE)) +
               wrap_elements(~testDispersion(quinn_resids))
...
quinn_brm3 |> conditional_effects("fSEASON:DENSITY") |> plot(points = TRUE)
```
...
quinn_brm3 |> ggpredict(~fSEASON+DENSITY) |> plot(show_data = TRUE)
```
...
quinn_brm3 |> ggemmeans(~fSEASON|DENSITY) |> plot(show_data = TRUE)
```
...
- if there were no interactions, we would expect the Low density Summer
  ...
  the modelled mean is `r -1*round(quinn_sum[6,1], 2)` less than the additive
...
quinn_tidy <- tidyMCMC(quinn_brm3$fit, estimate.method='median',  conf.int=TRUE,  conf.method='HPDinterval',  rhat=TRUE, ess=TRUE)
```
...
quinn_brm3 |> get_variables()
quinn_draw <- quinn_brm3 |>
    gather_draws(`.Intercept.*|.*fSEASON.*|.*DENSITY.*`,  regex = TRUE)
quinn_draw
...
quinn_gather <- quinn_brm3 |> gather_draws(`.Intercept.*|.*fSEASON.*|.*DENSITY.*`,  regex = TRUE) |>
  median_hdci()
```
...
- the estimated mean (expected number of newly recruited barnacles) on the ALG1
  surface is `r round(as.numeric(quinn_gather[1,2]),2)`.  This is the mean of the posterior distribution
  for this parameter.  If we back-transform this to the response scale, this
  becomes `r round(exp(as.numeric(quinn_gather[1, 2])),2)`.
- the estimated effect of ALG2 vs ALG1 is `r round(as.numeric(quinn_gather[2,2]),2)`
(median) with a standard error of `r round(as.numeric(quinn_gather[2,3]),2)`.
  The 95% credibility intervals indicate that we are 95% confident that the effect is between
  `r round(as.numeric(quinn_gather[2,4]),2)` and `r round(as.numeric(quinn_gather[2,5]),2)` - e.g. there is a
  significant positive effect.  When back-transformed onto the response scale, we see
  that barnacle recruitment on ALG2 is `r round(exp(as.numeric(quinn_gather[2,2])),2)` times
  higher than that on ALG1.
  This represents a `r round(100*(exp(as.numeric(quinn_gather[2, 2]))-1), 0)`% increase in
  barnacle recruitment.
 - the estimated effect of NB and S are `r round(as.numeric(quinn_gather[3, 2]), 2)` and
  `r round(as.numeric(quinn_gather[4, 2]), 2)` respectively, which equate to
  `r round(1/exp(as.numeric(quinn_gather[3,2])),2)` and  `r round(1/exp(as.numeric(quinn_gather[4,2])),2)` fold
...
quinn_brm3$fit |> plot(type='intervals')
```
...
quinn_brm3 |> rope(range = c(-0.28, 0.28))
rope(quinn_brm3, range = c(-0.28, 0.28)) |> plot()
...
#| echo: true
#| cache: false
quinn_brm3 |> modelplot(coef_omit = "shape", exponentiate = TRUE)
```
...
preds <- posterior_predict(quinn_brm4,  nsamples=250,  summary=FALSE)
quinn_resids <- createDHARMa(simulatedResponse = t(preds),
                            observedResponse = quinn$RECRUITS,
                            fittedPredictedResponse = apply(preds, 2, median),
                            integerResponse = TRUE)
plot(quinn_resids)
newdata = emmeans(quinn_brm4, ~fSEASON|DENSITY, type='response') |> as.data.frame()
newdata
