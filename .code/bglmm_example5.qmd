...
In the spirit of building up a model (rather than leap straight to a
more complex model that will be much more difficult to diagnose in the
event of any issues), we will start with a simple model.  This model will just have the offset and a random intercept term.
...
- $\beta_0$: normal centred at 0.3 with a standard deviation of 1.5
  - mean of 1.5: since `median(log(owls$NCalls / BroodSize))` or `mean(asinh(owls$NCalls/(2*1))/log(exp(1)))`
  - sd of 1.5: since `sd(log(owls$NCalls / BroodSize))` or `sd(asinh(owls$NCalls/(2*1))/log(exp(1)))`
...
owls |>
  mutate(calls_per_chick = (NCalls + 0.5) / BroodSize) |>
  summarise(
    mean = mean(log(calls_per_chick)),
    median = median(log(calls_per_chick)),
    SD = sd(log(calls_per_chick)),
    MAD = mad(log(calls_per_chick))
  )

priors <- prior(normal(0.3, 1.5), class = 'Intercept') +
...
## Cant view conditional effects plot for intercept only
## owls_brm1 |>
##     conditional_effects() |>
##   plot(points = TRUE)
...
owls_brm0 |>
    ...
    geom_pointrange(aes(ymin = lower.HPD, ymax = upper.HPD), color = "blue") +
    geom_point(
...
#| echo: true
#| cache: true
owls_brm1 <- update(owls_brm0, sample_prior = "yes", seed = 123)
owls_brm1 |>
    emmeans(~1, type = "response") |>
    as.data.frame() |>
    ggplot(aes(x = 1, y = rate)) +
...
```{r posterior2h_1caa, results='markdown', eval=TRUE, fig.width = 7, fig.height = 5}
owls_brm1 |> SUYR_prior_and_posterior()
```
...
```{r modelValidation1c_a, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
available_mcmc()
```
...
```{r modelValidation1c_b, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
pars <- owls_brm1 |> get_variables()
pars <- pars |> str_extract('^b.Intercept|^b_FoodTreatment.*|^b_SexParent.*|[sS]igma|^sd.*') |>
    na.omit()
pars
owls_brm1 |> mcmc_plot(type='trace', variable = pars)
##OR
owls_brm1 |> mcmc_plot(type='trace',
...
```{r modelValidation1c_c, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1 |> mcmc_plot(type='acf_bar', variable = pars)
##OR
owls_brm1 |> mcmc_plot(type='acf_bar',
                        variable = '^b.Intercept|^b_FoodTreatment.*|^b_SexParent.*|[sS]igma|^sd.*',
                        regex = TRUE)
```
...
```{r modelValidation1c_d, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1 |> mcmc_plot(type='rhat_hist')
```
...
```{r modelValidation1c_e, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1 |> mcmc_plot(type='neff_hist')
```

  Ratios all very high.

<details><summary>More diagnostics</summary>
```{r modelValidation1c_f, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1 |> mcmc_plot(type='combo', pars = pars)
owls_brm1 |> mcmc_plot(type='violin', pars = pars)
```
</details>
...
```{r modelValidation1c_g, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
...
```{r modelValidation1c_h, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1$fit |>
    stan_ac(pars = pars)
```
...
```{r modelValidation1c_i, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1$fit |> stan_rhat()
```
...
```{r modelValidation1c_j, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1$fit |> stan_ess()
```

  Ratios all very high.

```{r modelValidation1c_k, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1$fit |>
...
```{r modelValidation1c_l, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=7}
...
```{r modelValidation1c_m, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=7}
...
```{r modelValidation1c_n, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## ggs_Rhat(owls_ggs)
```
...
```{r modelValidation1c_o, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
...
<details><summary>More diagnostics</summary>
```{r modelValidation1c_p, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## ggs_crosscorrelation(owls_ggs)
```

```{r modelValidation1c_q, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## ggs_grb(owls_ggs)
```
</details>
...
```{r modelValidation1c_r, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
available_ppc()
```
...
```{r modelValidation1c_s, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1 |> pp_check(type = 'dens_overlay', ndraws = 100)
```
...
```{r modelValidation1c_t, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## owls_brm1 |> pp_check(type = 'error_scatter_avg')
```
...
```{r modelValidation1c_u, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm1 |> pp_check(group = 'Nest', type = 'intervals')
```
...
```{r modelValidation1c_v, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
#library(shinystan)
...
```{r modelValidation1c_w, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=8}
owls_resids <- make_brms_dharma_res(owls_brm1, integerResponse = TRUE)
...
```{r validation1c_x, results='markdown', eval=TRUE, error=TRUE,mhidden=TRUE, fig.width=7, fig.height=5, cache=FALSE, message=FALSE, warning=FALSE}
...
```{r validation1c_y, results='markdown', eval=TRUE, error=TRUE,mhidden=TRUE, fig.width=7, fig.height=5, cache=TRUE, message=FALSE, warning=FALSE}
...
```{r fitModel2h_2a, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
owls_form <- bf(NCalls ~ +
                  offset(log(BroodSize)) + (1|Nest),
  zi = ~ 1,
...
```{r fitModel2h_2b, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
...
```{r modelValidation2c_g, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
...
owls_brm2b$fit |>
    stan_trace(pars = pars)
```
```{r modelValidation2c_h, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm2b$fit |>
    stan_ac(pars = pars)
```
```{r modelValidation2c_i, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm2b$fit |> stan_rhat()
```
```{r modelValidation2c_j, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm2b$fit |> stan_ess()
```
```{r modelValidation2c_k, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm2b$fit |>
    stan_dens(separate_chains = TRUE, pars = pars)
```
...
```{r modelValidation2c_s, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm2b |> pp_check(type = 'dens_overlay', ndraws = 100)
```

```{r modelValidation2c_t, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## owls_brm2b |> pp_check(type = 'error_scatter_avg')
```

```{r modelValidation2c_u, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm2b |> pp_check(group = 'Nest', type = 'intervals')
```

##### DHARMa residuals

```{r modelValidation2c_w, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=8}
...
:::: {.panel-tabset}
#### Fit model
```{r fitModel2h_3a, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
owls_form <- bf(NCalls ~ +
                  offset(log(BroodSize)) + (1|Nest),
  zi = ~ 1,
...
```{r fitModel2h_3b, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
...
```{r modelValidation3c_g, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
...
owls_brm3b$fit |>
    stan_trace(pars = pars)
```
```{r modelValidation3c_h, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm3b$fit |>
    stan_ac(pars = pars)
```
```{r modelValidation3c_i, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm3b$fit |> stan_rhat()
```
```{r modelValidation3c_j, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm3b$fit |> stan_ess()
```
```{r modelValidation3c_k, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm3b$fit |>
    stan_dens(separate_chains = TRUE, pars = pars)
```
...
```{r modelValidation3c_s, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm3b |> pp_check(type = 'dens_overlay', ndraws = 100)
```

```{r modelValidation3c_t, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## owls_brm3b |> pp_check(type = 'error_scatter_avg')
```

```{r modelValidation3c_u, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm3b |> pp_check(group = 'Nest', type = 'intervals')
```

##### DHARMa residuals

```{r modelValidation3c_w, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=8}
...
**Add fixed effect structure**

:::: {.panel-tabset}
#### Fit model

```{r fitModel2h_4a, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
...
```{r fitModel2h_4b, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
...
owls_brm4b |>
  conditional_effects("FoodTreatment:SexParent") |>
  plot(points = TRUE, point_args = list(width = 0.1))
```
...
```{r modelValidation4c_g, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
...
owls_brm4b$fit |>
    stan_trace(pars = pars)
```
```{r modelValidation4c_h, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm4b$fit |>
    stan_ac(pars = pars)
```
```{r modelValidation4c_i, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm4b$fit |> stan_rhat()
```
```{r modelValidation4c_j, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm4b$fit |> stan_ess()
```
```{r modelValidation4c_k, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm4b$fit |>
    stan_dens(separate_chains = TRUE, pars = pars)
```
...
```{r modelValidation4c_s, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm4b |> pp_check(type = 'dens_overlay', ndraws = 100)
```

```{r modelValidation4c_t, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## owls_brm4b |> pp_check(type = 'error_scatter_avg')
```

```{r modelValidation4c_u, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm4b |> pp_check(group = 'Nest', type = 'intervals')
```

##### DHARMa residuals

```{r modelValidation4c_w, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=8}
...
**Explore random slopes and more complex zero-inflation**

:::: {.panel-tabset}
#### Fit model
```{r fitModel2h_5a, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
owls_form <- bf(NCalls ~ FoodTreatment*SexParent +
...
```{r fitModel2h_5b, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
owls_brm5b <- update(owls_brm5a, sample_prior = "yes",
                       iter =  5000,
                       warmup =  2500,
                       control = list(adapt_delta = 0.99, max_treedepth = 20),
                       backend =  "cmdstanr",
...
```{r modelValidation5c_g, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
...
owls_brm5b$fit |>
    stan_trace(pars = pars)
```
```{r modelValidation5c_h, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm5b$fit |>
    stan_ac(pars = pars)
```
```{r modelValidation5c_i, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm5b$fit |> stan_rhat()
```
```{r modelValidation5c_j, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm5b$fit |> stan_ess()
```
```{r modelValidation5c_k, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm5b$fit |>
    stan_dens(separate_chains = TRUE, pars = pars)
```
...
```{r modelValidation5c_s, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm5b |> pp_check(type = 'dens_overlay', ndraws = 100)
```

```{r modelValidation5c_t, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## owls_brm5b |> pp_check(type = 'error_scatter_avg')
```

```{r modelValidation5c_u, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm5b |> pp_check(group = 'Nest', type = 'intervals')
```

##### DHARMa residuals

```{r modelValidation5c_w, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=8}
...
**Explore zero-inflated negative binomial**

:::: {.panel-tabset}
#### Fit model
```{r fitModel2h_6a, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
owls_form <- bf(NCalls ~ FoodTreatment*SexParent +
                  offset(log(BroodSize)) +
                  (FoodTreatment*SexParent|Nest),
  zi = ~ FoodTreatment*SexParent,
...
```{r fitModel2h_6b, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
owls_brm6b <- update(owls_brm6a, sample_prior = "yes",
                       iter =  5000,
                       warmup =  2500,
                       control = list(adapt_delta = 0.99, max_treedepth = 20),
                       backend =  "cmdstanr",
...
```{r posterior2h_6c, results='markdown', eval=TRUE, fig.width = 7, fig.height = 5}
## owls_brm5b |> SUYR_prior_and_posterior()
```

#### MCMC sampling diagnostics

```{r modelValidation6c_g, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm6b |> get_variables()
pars <- owls_brm6b |> get_variables()
pars <- str_extract(pars, '^b_.*|^sigma$|^sd.*') |> na.omit()

owls_brm6b$fit |>
    stan_trace(pars = pars)
```
```{r modelValidation6c_h, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm6b$fit |>
    stan_ac(pars = pars)
```
```{r modelValidation6c_i, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm6b$fit |> stan_rhat()
```
```{r modelValidation6c_j, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm6b$fit |> stan_ess()
```
```{r modelValidation6c_k, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm6b$fit |>
    stan_dens(separate_chains = TRUE, pars = pars)
```
...
```{r modelValidation6c_s, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm6b |> pp_check(type = 'dens_overlay', ndraws = 100)
```

```{r modelValidation6c_t, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
## owls_brm6b |> pp_check(type = 'error_scatter_avg')
```

```{r modelValidation6c_u, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
owls_brm6b |> pp_check(group = 'Nest', type = 'intervals')
```

##### DHARMa residuals

```{r modelValidation6c_w, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=8}
owls_resids <- make_brms_dharma_res(owls_brm6b, integerResponse = TRUE)
wrap_elements(~testUniformity(owls_resids)) +
               wrap_elements(~plotResiduals(owls_resids, form = factor(rep(1, nrow(owls))))) +
               wrap_elements(~plotResiduals(owls_resids, quantreg = TRUE)) +
               wrap_elements(~testDispersion(owls_resids))
...
```{r modelValidation7a_1, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=8}
(loo_3 <- loo::loo(owls_brm3b))
(loo_4 <- loo::loo(owls_brm4b))
(loo_5 <- loo::loo(owls_brm5b))
(loo_6 <- loo::loo(owls_brm6b))
loo::loo_compare(loo_3, loo_4, loo_5, loo_6)

```

Conclusions:
- evidently, the zero-inflated negative binomial model with random
  intercepts/slopes (owls_brm6b) is the best model...
...
owls_sum <- summary(owls_brm6b)
```

**Conclusions:**

- the average number of calls made per food deprived owl chicks when the female
  parent returns to the nest is `r as.numeric(round(owls_sum$fixed[1, 1],2))` (on the
  link scale). When back-transformed, this is `r as.numeric(round(exp(owls_sum$fixed[1, 1]),2))`.
- on average, satiated chicks negotiate
  `r as.numeric(round(owls_sum$fixed[3,1]),2)` (link scale) less when the female
  returns than deprived chicks.  This equates to
  `r as.numeric(round(exp(owls_sum$fixed[3,1]),2))` fold fewer times and represents a
  `r as.numeric(100*(1-round(exp(owls_sum$fixed[3,1]),2)))`% decline.
- on average, when the male parent returns, deprived chicks negotiate
  `r as.numeric(round(owls_sum$fixed[4,1]),2)` (link scale) less when the female
  returns.  This equates to `r as.numeric(round(exp(owls_sum$fixed[4,1]),2))` fold
  fewer times and represents a
  `r as.numeric(100*(1-round(exp(owls_sum$fixed[4,1]),2)))`% decline (although this
  is not significant).
...
- the estimated rate of non-detection of calls (false zeros) for deprived chicks
  when the female parent returns is `r as.numeric(round(owls_sum$fixed[2,1],2))`
  (logit scale).  When back-transformed to the odds scale, this equates to the
  odds of a zero being false are `r as.numeric(round(exp(owls_sum$fixed[2,1]),2))`:1.
  Expressed on a probability scale, it would suggest that the probability of a
  zero being false is `r round(plogis(as.numeric(owls_sum$fixed[2,1])),2)`.
- when the chicks are satiated, the odds not detecting a call (false zero) are
  increased `r as.numeric(round(exp(owls_sum$fixed[6,1]),2))` fold.  That is, the odds
  increase by `r round(100*(1-plogis(as.numeric(owls_sum$fixed[6,1]))), 2)`%.
- when the returning parent is male (for deprived chicks), the odds of not
  detecting a call (false zeros) are reduced
  `r as.numeric(round(exp(owls_sum$fixed[7,1]),2))` fold.  That is, the odds decline
  by `r round(100*(1-plogis(as.numeric(owls_sum$fixed[7,1]))), 2)`%.
...
owls_tidy <- tidyMCMC(owls_brm6b$fit, estimate.method='median',
                         conf.int=TRUE,  conf.method='HPDinterval',
                         rhat=TRUE, ess=TRUE)
```
...
owls_brm6b$fit |> plot(type='intervals')
```
...
#| echo: true
#| cache: false
owls_brm6b |> modelplot(exponentiate = TRUE)
```
::::
:::
<!-- END_PRIVATE-->
...
ggplot(newdata, aes(y=prob, x=as.numeric(SexParent)) +
  ...
  owls_brm8 |>
      emmeans(~ FoodTreatment + SexParent, type = "response", offset = log(1)) |>
    as.data.frame() |>
    ggplot(aes(x = FoodTreatment, y = rate, color = SexParent)) +
      geom_pointrange(aes(ymin = lower.HPD, ymax = upper.HPD),
                      position = position_dodge(width = 0.2)) +
      theme_classic() +
      labs(y = "Predicted number of calls", x = "Food treatment")
