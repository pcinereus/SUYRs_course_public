...
polis_brm0 |> ggemmeans(~RATIO) |> plot(show_data=TRUE)
polis_brm0 |> conditional_effects() |>  plot(points=TRUE)
```
...
So if the predictors are centered (or scaled), we might expect the
intercept to represent a probability of approximately 0.5 (and thus
presumably close to the switch point). This would seem inappropriate.
When this is the case, we want to use a standard deviation of the
intercept prior of 1.

On the other hand, if we do not scale the predictor, we might expect
the intercept to represent a probability close to either 0 or 1. When
this is the case, we want to use a standard deviation of the intercept
prior of 2.5.

When we want to obtain a ED50 (or LD50), it is easier to not have
scaled the predictor.  So in this instance we will not scale the predictor and thus use a standard devation for the intercept prior of 1.
...
polis_brm1 |> conditional_effects() |>
  plot(points = TRUE)
```

<!-- IGNORE_START-->
### ggemmeans
```{r fitModel2iaa, results='markdown', eval=TRUE, mhidden=TRUE, cache=FALSE}
ggemmeans(polis_brm1,  ~RATIO) |>
  plot(show_data = TRUE)
```
<!-- IGNORE_END-->
...
polis_brm2 <- polis_brm1 |> update(sample_prior = 'yes', refresh = 0, seed = 123)
```
...
polis_brm2 |> conditional_effects(method = "posterior_linpred")
...
### ggemmeans
```{r fitModel2j2aa, results='markdown', eval=TRUE, mhidden=TRUE}
ggemmeans(polis_brm2,  ~RATIO) |>
  plot(show_data = TRUE)
ggemmeans(polis_brm2,  terms = "RATIO[0:63]") |>
    plot(show_data = TRUE)
```
<!-- IGNORE_END-->
:::
...
polis_brm2 |> get_variables()
## polis_brm2 |> hypothesis('Intercept=0', class='b') |> plot()
polis_brm2 |> hypothesis('RATIO=0') |> plot()
```

```{r fitModel2k, results='markdown', eval=TRUE, mhidden=TRUE, cache=FALSE}
polis_brm2 |> get_variables()
polis_brm2 |> SUYR_prior_and_posterior()
```
...
ggs_effective(polis.ggs, version_effective = "BDA3")
```
...
polis_brm2 |> mcmc_plot(type = 'trace')
```
...
polis_brm2 |> mcmc_plot(type = 'acf_bar')
```
...
polis_brm2 |> mcmc_plot(type = 'rhat_hist')
```
...
polis_brm2 |> mcmc_plot(type = 'neff_hist')
```
...
polis_brm2 |> mcmc_plot(type = 'combo')
polis_brm2 |> mcmc_plot(type = 'violin')
```
</details>
...
polis.ggs <- polis_brm2 |> ggs(inc_warmup = FALSE, burnin = FALSE)
...
polis.ggs |> ggs_effective(version_effective = "BDA3")
```
...
polis_brm2 |> pp_check(type = 'dens_overlay', ndraws=100)
```
The model draws appear to be consistent with the observed data.
...
## polis_brm2 |> pp_check(type = 'error_scatter_avg')
```
...
polis_brm2 |> pp_check(x = 'RATIO', type = 'error_scatter_avg_vs_x')
```
...
polis.resids <- make_brms_dharma_res(polis_brm2, integerResponse = FALSE)
...
Here is the old (and now incorrect) way...

```{r modelValidation6a, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
preds <- polis_brm2 |> posterior_predict(ndraws = 250,  summary = FALSE)
polis.resids <- createDHARMa(simulatedResponse = t(preds),
                            observedResponse = polis$PA,
                            fittedPredictedResponse = apply(preds, 2, median),
                            integerResponse = TRUE)
...
polis_brm2 |> epred_draws(newdata = polis) |>
 median_hdci() |>
 ggplot(aes(x = RATIO, y = .epred)) +
 geom_ribbon(aes(ymin = .lower, ymax = .upper), fill = 'blue', alpha = 0.3) +
 geom_line()
...
partial.obs <- polis |>
    mutate(fit = fitted(polis_brm2, newdata = polis)[,'Estimate'],
           resid = resid(polis_brm2)[,'Estimate'],
           Obs = fit + resid)
...
- in the Model Info, we are informed that the total MCMC posterior sample size
  is `r nrow(as.matrix(polis_brm2))` and that there were 19 raw observations.
- the estimated intercept (expected presence/absence when perimeter to area ratio is 0) is
...
modelsummary(list("Raw" = polis_brm2, "Exponentiated" = polis_brm2),
  statistic = c("conf.low", "conf.high"),
  shape = term ~ statistic,
...
#| echo: true
#| cache: false
polis_brm2 |> modelplot()
polis_brm2 |> modelplot(exponentiate = TRUE)
```
...
polis.tidy <- polis_brm2$fit |> tidyMCMC(estimate.method = 'median',  conf.int = TRUE,  conf.method = 'HPDinterval',  rhat = TRUE, ess = TRUE)
```
...
polis.draw <- polis_brm2 |> gather_draws(b_Intercept, b_RATIO)
## OR via regex
polis.draw <- polis_brm2 |> gather_draws(`b_.*`,  regex=TRUE)
polis.draw
```
...
polis_brm2 |> spread_draws(b_Intercept, b_RATIO)
# OR via regex
polis_brm2 |> spread_draws(`b_.*`,  regex=TRUE)
...
polis_brm2 |> posterior_samples() |> as_tibble()
```
...
polis_brm2 |> mcmc_plot(type = 'intervals')
```
...
partial.obs <- polis |>
    bind_cols(Pred = predict(polis_brm2)[,'Estimate'],
              Resid = residuals(polis_brm2)[,'Estimate']) |>
    mutate(
...
## spaghetti plot
newdata = emmeans(polis_brm2, ~RATIO, at=polis_grid) |>
    gather_emmeans_draws() |>
    mutate(.value = plogis(.value))
newdata |> head()
ggplot(newdata,  aes(y=.value,  x=RATIO)) +
