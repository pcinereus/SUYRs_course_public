...
This becomes the log mean of the lognormal prior
...
priors <- prior(normal(-1.1, 0.6), class = "Intercept") +
  ...
  prior(lognormal(log(0.35), 0.5), class = "lscale", coef = "gpREPORT_YEAR") +
  ## prior(exponential(5), class = "lscale", coef = "gpREPORT_YEAR") +
  prior(student_t(3, 0, 0.6), class = "sdgp")
...
```{r gp mcmc diagnostics 1, results='markdown', eval=TRUE, fig.width=6, fig.height=4}
pars <- ltmp_sub_brm3 |>
    get_variables() |>
    str_subset("^b_.*|^sd.*")
pars
...
```{r gp mcmc diagnostics 2, results='markdown', eval=TRUE, fig.width=6, fig.height=4}
ltmp_sub_brm3$fit |>
    stan_ac(pars = pars)
```

```{r gp mcmc diagnostics 3, results='markdown', eval=TRUE, fig.width=6, fig.height=4}
ltmp_sub_brm3$fit |> stan_rhat()
```

```{r gp mcmc diagnostics 4, results='markdown', eval=TRUE, fig.width=6, fig.height=4}
ltmp_sub_brm3$fit |> stan_ess()
```

```{r gp mcmcm diagnostics 5, results='markdown', eval=TRUE, fig.width=6, fig.height=4}
ltmp_sub_brm3$fit |>
    stan_dens(separate_chains = TRUE, pars = pars)
```
...
```{r gp model validation 1, results='markdown', eval=TRUE, fig.width=6, fig.height=4}
ltmp_sub_brm3 |> pp_check(type = 'dens_overlay', ndraws = 100)
```

```{r gp model validation 2, results='markdown', eval=TRUE, fig.width=8, fig.height=10}
ltmp_sub_resids <- make_brms_dharma_res(ltmp_sub_brm3, integerResponse = TRUE)
wrap_elements(~testUniformity(ltmp_sub_resids)) +
               wrap_elements(~plotResiduals(ltmp_sub_resids, form = factor(rep(1, nrow(ltmp_sub))))) +
               wrap_elements(~plotResiduals(ltmp_sub_resids, quantreg = TRUE)) +
               wrap_elements(~testDispersion(ltmp_sub_resids))
