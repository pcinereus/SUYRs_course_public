...
hughes |>
    ggplot(aes(y = oSCORE, x = HABITAT)) +
  geom_point(position = position_jitter()) +
  geom_line(aes(y = as.numeric(oSCORE), x = as.numeric(HABITAT), group = REEF),
...
hughes_form <- bf(oSCORE ~ HABITAT*SECTOR + (1|REEF),
  family = cumulative(link = "logit", threshold = "flexible")
)
options(width=150)
get_prior(hughes_form, data = hughes)
options(width=80)
```
...
hughes_form <- bf(oSCORE ~ HABITAT*SECTOR + (HABITAT*SECTOR|REEF),
  family = cumulative(link = "logit", threshold = "flexible")
)
...
hughes_brm1 |>
  conditional_effects("HABITAT",
    conditions = make_conditions(hughes_brm1, "SECTOR"),
    categorical = TRUE
)
```
...
hughes_brm2 <- update(hughes_brm1,
                       sample_prior = 'yes',
                       chains = 3, cores = 3,
                       seed = 123,
                       refresh = 100)
...
hughes_brm2 |>
  conditional_effects("HABITAT",
    conditions = make_conditions(hughes_brm2, "SECTOR"),
    categorical = TRUE
)
...
hughes_brm2 |> get_variables()
hughes_brm2 |> hypothesis('HABITATF=0') |> plot()
hughes_brm2 |> hypothesis('SECTORCentral=0') |> plot()
```

```{r posterior2h2a, results='markdown', eval=TRUE, fig.width = 7, fig.height = 5}
## hughes_brm2 %>% SUYR_prior_and_posterior()
```
...
pars <- hughes_brm2 %>% get_variables()
pars <- hughes_brm2 |>
    get_variables() |>
    str_subset("^b_.*|[sS]igma|^sd.*")
pars
hughes_brm2 |> mcmc_plot(type = 'trace', variable = pars)
```
...
hughes_brm2 |> mcmc_plot(type='acf_bar', variable = pars)
```
...
hughes_brm2 |> mcmc_plot(type='rhat_hist')
```
...
hughes_brm2 |> mcmc_plot(type='neff_hist')
```
...
hughes_brm2 |> mcmc_plot(type='combo', variable = pars)
hughes_brm2 |> mcmc_plot(type='violin', variable = pars)
```
</details>
...
hughes_brm2 |> get_variables() |>  head(n = 30)
pars <- hughes_brm2 |>
    get_variables() |>
    str_subset("^b_.*|[sS]igma|^sd.*")
pars
...
hughes_brm2 |> pp_check(type = 'dens_overlay', ndraws = 100)
```
...
## hughes_brm2 %>% pp_check(type = 'error_scatter_avg')
```
...
hughes_brm2 %>% pp_check(group = 'REEF', type = 'intervals')
```
...
The following will be slow

```{r modelValidation6aa, results='markdown', eval=TRUE, fig.width=8, fig.height=10}
hughes.resids <- make_brms_dharma_res(hughes_brm2, integerResponse = TRUE)
...
hughes_sum <- summary(hughes_brm2)
```
...
- -0.35 (0.7) odds of being in a higher category are
30% lower in the F habitat than the C habitat (In the North)
- -1.34 (0.26) odds of being in a higher category are
64% lower in the L habitat than the C habitat (in the North)
- 1.20 (3.3) odds of being in a higher category are
232% higher in the U habitat than the C habitat (in the North)
- -1.72 (0.179) odds of being in a higher category are
82% lower in central vs northern (for C habitat)
- -6.47 (0.0015) odds of being in a higher category are
99% lower in south vs northern (for C habitat)

### as_draws_df (posteriors)

```{r summariseModel2bm, results='markdown', eval=TRUE, fig.width=8, fig.height=5,echo=TRUE}
hughes_brm2 |> as_draws_df()
hughes_brm2 |>
    as_draws_df() |>
    ## exp() |>
...
newdata <- hughes |>
  distinct(HABITAT, SECTOR)
newdata
predict(hughes_brm2, newdata = newdata, re_formula = NA)

sum(0:4 * (c(0.0000000000, 0.0041666667, 0.04666667, 0.35000000, 0.5991666667)))

epred_draws(hughes_brm2, newdata = newdata, re_formula = NA) |>
  head(n = 10)

epred_draws(hughes_brm2,
    newdata = newdata,
    re_formula = NA
) |>
  ## filter(.draw == 1, HABITAT == "C", SECTOR == "North") |>
...
## Alternative
## marginaleffects::avg_predictions(hughes_brm2, newdata = newdata, c("SECTOR", "HABITAT"), re_formula = NA) |>
...
## Comparing absolute change in prob
epred_draws(hughes_brm2,
    newdata = newdata,
    re_formula = NA
) |>
...
## Comparing relative change in prob
hughes_eff <-
  epred_draws(hughes_brm2,
    newdata = newdata,
    re_formula = NA
...
```{r, eval=FALSE}
hughes_form <- bf(oSCORE ~ HABITAT*SECTOR + (HABITAT|REEF),
  family = cumulative(link = "logit", threshold = "flexible")
)
hughes_form <- bf(oSCORE ~ HABITAT*SECTOR + (HABITAT+HABITAT:SECTOR|REEF),
  family = cumulative(link = "logit", threshold = "flexible")
)
get_prior(hughes_form, data = hughes)
...
hughes.resids <- make_brms_dharma_res(hughes_brm4, integerResponse = TRUE)
wrap_elements(~testUniformity(hughes.resids)) +
               wrap_elements(~plotResiduals(hughes.resids, form = factor(rep(1, nrow(hughes))))) +
               wrap_elements(~plotResiduals(hughes.resids, quantreg = TRUE)) +
               wrap_elements(~plotResiduals(hughes.resids, quantreg = TRUE, asFactor = FALSE)) +
...
hughes_brm4 |> bayes_R2(re.form = NA, summary=FALSE) |>
    median_hdci()
hughes_brm4 |> bayes_R2(re.form = ~ (1|REEF), summary=FALSE) |>
    median_hdci()
hughes_brm4 |> bayes_R2(re.form = ~ (1|REEF), summary=FALSE) |>
    median_hdci()
```
...
## Cellmeans
add_epred_draws(hughes_brm2, newdata = newdata, re_formula = NA) |>
  mutate(fit = as.numeric(as.character(.category)) * .epred) |>
  group_by(HABITAT, SECTOR, .draw) |>
  summarise(fit = sum(fit)) |>
  summarise_draws(
...
sum(0:4 * posterior_epred(hughes_brm2, newdata = newdata, re_formula = NA)[,4,] |> colMeans())
...
hughes_brm2 |> emmeans(~HABITAT | SECTOR, mode = "latent")
...
newdata <- cbind(newdata, predict(hughes_brm2, newdata = newdata))
newdata
