...
\beta_0 &\sim{} \mathcal{N}(3.4, 0.1)\\
\beta_{1-9} &\sim{} \mathcal{N}(0, 2)\\
\sigma &\sim{} \mathcal{t}(3,0, 1.5)
\end{align}
$$
...
##loyn_brm0 |> ggemmeans(~AREA) |> plot(show_data = TRUE) + scale_y_log10()
loyn_brm0 |>
    ggemmeans(~AREA) |>
    plot(show_data = TRUE) + scale_y_log10()
```
...
- $\beta_1$: normal centred at 0 with a standard deviation of 2
  (again, consider what this would be on a response scale)
  - sd of 2: since `tapply(log(loyn$ABUND), loyn$fGRAZE, FUN = "median")`
    (and since each predictor is scaled, it will be the same for each
    predictor)
- $\sigma$: half t distribution (flat Gaussian) prior with a scale of
  1.5 since that would encapsulate the maximum mad of any group.
...
loyn_brm2 <- update(loyn_brm1,  sample_prior = 'yes', refresh = 0)
```
...
## loyn_brm2 |> hypothesis('Intercept = 0', class = 'b') |> plot
## loyn_brm2 |> hypothesis('Intercept = 0', class = 'prior') |> plot
loyn_brm2 |> hypothesis('scalelogDIST = 0') |> plot()
loyn_brm2 |> hypothesis('scalelogAREA = 0') |> plot()
loyn_brm2 |> hypothesis('sigma = 0', class = '') |> plot()
```

```{r fitModel2k2, results='markdown', eval=TRUE, mhidden=TRUE, fig.width = 8, fig.height = 4}
loyn_brm2 |> SUYR_prior_and_posterior()
```
...
## ggs_effective(loyn_ggs, version_effective = "BDA3")
```
...
loyn_brm2 |> mcmc_plot(type = 'trace')
```
...
loyn_brm2 |> mcmc_plot(type = 'acf_bar')
```
...
loyn_brm2 |> mcmc_plot(type = 'rhat_hist')
```
...
loyn_brm2 |> mcmc_plot(type = 'neff_hist')
```
...
loyn_brm2 |> mcmc_plot(type = 'combo')
loyn_brm2 |> mcmc_plot(type = 'violin')
```
</details>
...
loyn_ggs <- loyn_brm2 |> ggs(inc_warmup = FALSE, burnin = FALSE)
...
loyn_ggs |> ggs_effective(version_effective = "BDA3")
```
...
loyn_brm2 |> pp_check(type = 'dens_overlay', ndraws = 100)
```
...
#loyn_brm2 |> pp_check(type = 'error_scatter_avg')
```
...
loyn_brm2 |> pp_check(x = 'AREA', type = 'error_scatter_avg_vs_x')
```
...
loyn_brm2 |> pp_check(x = 'AREA', type = 'intervals')
```
...
loyn_brm2 |> pp_check(x = 'AREA', type = 'ribbon')
```
...
preds <- loyn_brm2 |> posterior_predict(ndraws = 250,  summary = FALSE)
loyn_resids <- createDHARMa(simulatedResponse = t(preds),
                            observedResponse = loyn$ABUND,
                            fittedPredictedResponse = apply(preds, 2, median),
                            integerResponse = FALSE)
...
loyn_resids <- make_brms_dharma_res(loyn_brm2, integerResponse = FALSE)
...
- in the Model Info, we are informed that the total MCMC posterior sample size
  is `r nrow(as.matrix(loyn_rstanarm3))` and that there were 56 raw observations.
- the estimated intercept (expected bird abundance when grazing intensity is
  ...
  `r round(loyn_sum[1,1],2)`.  This is the mean of the posterior distribution
  for this parameter.  If we back-transform this to the response scale, this
  becomes `r round(exp(loyn_sum[1, 1]),2)`.
- the estimated slope associated with `DIST` (rate at which the abundance of
  ...
  and grazing intensity at level 1), is `r round(loyn_sum[2,1],2)` (mean) or
  `r round(loyn_sum[2,4],2)` (median) with a standard deviation of `r round(loyn_sum[2,2],2)`.
  The 90% credibility intervals indicate that we are 90% confident that the slope is between
  `r round(loyn_sum[2,1],2)` and `r round(loyn_sum[2,5],2)` - e.g. there is no
  evidence of a
  significant trend.
...
in centred log patch Area, bird abundance is expected to increase by (log) `r round(I(loyn_sum[4, 1]),2)`.
- if we back transform this slope (by exponentiation), we get a partial slope for centred log Area of `r round(exp(loyn_sum[4, 1]),2)`. This is interpreted as - for every 1 unit increase in (scaled log) Area, the bird abundance is expected to increase `r round(exp(loyn_sum[4, 1]),2)` fold. That is, there is a `r round(100*(exp(loyn_sum[4, 1])-1),2)` percent increase per 1 unit increase in centred log Area.
...
- the estimated intercept (expected bird abundance when grazing intensity is
  equal to 1 and all of the continuous
  predictors are at their respective averages) is
  `r round(as.numeric(loyn_tidy[1,1]),2)`.  This is the mean of the posterior distribution
  for this parameter.  If we back-transform this to the response scale, this
  becomes `r round(exp(as.numeric(loyn_tidy[1, 1])),2)`.
- the estimated slope associated with `DIST` (rate at which the abundance of
  birds changes per 1 unit change in
  log-transformed Distance to nearest patch holding other continuous predictors constant
  and grazing intensity at level 1), is `r round(as.numeric(loyn_tidy[2,1]),2)` (mean) or
  `r round(as.numeric(loyn_tidy[2,4]),2)` with a standard deviation of `r round(as.numeric(loyn_tidy[2,2]),2)`.
  The 90% credibility intervals indicate that we are 90% confident that the slope is between
  `r round(as.numeric(loyn_tidy[2,1]),2)` and `r round(as.numeric(loyn_tidy[2,5]),2)` - e.g. there is no
  evidence of a
  significant trend.
- associated with each of the continuous predictors is a partial slope. Each
partial slope is the rate of change between bird abundance and the associated
predictor (on the log scale due to the link and based on 1 unit change in the
predictor on the scale of the predictor). For example, for every one unit change
in centred log patch Area, bird abundance is expected to increase by (log) `r round(I(as.numeric(loyn_tidy[4, 1])),2)`.
- if we back transform this slope (by exponentiation), we get a partial slope for centred log Area of `r round(exp(as.numeric(loyn_tidy[4, 1])),2)`. This is interpreted as - for every 1 unit increase in (scaled log) Area, the bird abundance is expected to increase `r round(exp(as.numeric(loyn_tidy[4, 1])),2)` fold. That is, there is a `r round(100*(exp(as.numeric(loyn_tidy[4, 1]))-1),2)` percent increase per 1 unit increase in centred log Area.
- Rhat and number of effective samples for each parameter are also provided as
  MCMC diagnostics and all look good.
...
- the estimated intercept (expected bird abundance when grazing intensity is
  equal to 1 and all of the continuous
  predictors are at their respective averages) is
  `r round(loyn_sum$fixed[1,1],2)`.  This is the mean of the posterior distribution
  for this parameter.  If we back-transform this to the response scale, this
  becomes `r round(exp(loyn_sum$fixed[1, 1]),2)`.
- the estimated slope associated with `DIST` (rate at which the abundance of
  birds changes per 1 unit change in
  log-transformed Distance to nearest patch holding other continuous predictors constant
  and grazing intensity at level 1), is `r round(loyn_sum$fixed[2,1],2)` (mean) or
  `r round(loyn_sum$fixed[2,4],2)` (median) with a standard deviation of `r round(loyn_sum$fixed[2,2],2)`.
  The 90% credibility intervals indicate that we are 90% confident that the slope is between
  `r round(loyn_sum$fixed[2,1],2)` and `r round(loyn_sum$fixed[2,5],2)` - e.g. there is no
  evidence of a
  significant trend.
- associated with each of the continuous predictors is a partial slope. Each
partial slope is the rate of change between bird abundance and the associated
predictor (on the log scale due to the link and based on 1 unit change in the
predictor on the scale of the predictor). For example, for every one unit change
in centred log patch Area, bird abundance is expected to increase by (log) `r round(I(loyn_sum$fixed[4, 1]),2)`.
- if we back transform this slope (by exponentiation), we get a partial slope for centred log Area of `r round(exp(loyn_sum$fixed[4, 1]),2)`. This is interpreted as - for every 1 unit increase in (scaled log) Area, the bird abundance is expected to increase `r round(exp(loyn_sum$fixed[4, 1]),2)` fold. That is, there is a `r round(100*(exp(loyn_sum$fixed[4, 1])-1),2)` percent increase per 1 unit increase in centred log Area.
- Rhat and number of effective samples for each parameter are also provided as
  MCMC diagnostics and all look good.
...
loyn_tidy <- tidyMCMC(loyn_brm2$fit, estimate.method='median',  conf.int=TRUE,  conf.method='HPDinterval',  rhat=TRUE, ess=TRUE)
```
...
loyn_draw <- loyn_brm2 |> gather_draws(`^b_.*`,  regex = TRUE)
loyn_draw
```
...
loyn_brm2 |> mcmc_plot(type = 'intervals')
```
...
loyn_brm2 |> bayestestR::rope_range()
loyn_brm2 |> bayestestR::rope(range = c(-0.09, 0.09))
loyn_brm2 |> bayestestR::rope(range = c(-0.09, 0.09)) |>
    plot(data = loyn_brm2)
```
...
loyn_brm3a <- update(loyn_brm2,  .~scale(log(DIST))*scale(log(LDIST)),
                     save_pars = save_pars(all = TRUE), refresh = 0)
loyn_brm3b <- update(loyn_brm2,  .~scale(log(AREA)) * fGRAZE,
                     save_pars = save_pars(all = TRUE), refresh = 0)
loyn_brm3c <- update(loyn_brm2,  .~scale(log(AREA)) * fGRAZE * scale(YR.ISOL),
                     save_pars = save_pars(all = TRUE), refresh = 0)
loyn_brm3d <- update(loyn_brm2,  .~scale(ALT),
                     save_pars = save_pars(all = TRUE), refresh = 0)
loyn_brm3e <- update(loyn_brm2,  .~1,
                     save_pars = save_pars(all = TRUE), refresh = 0)
waic(loyn_brm3a)
loo(loyn_brm3a)
loo(loyn_brm3e)
loo_compare(loo(loyn_brm3a),
            loo(loyn_brm3e)
            )
loo_compare(loo(loyn_brm3b),
            loo(loyn_brm3e)
            )
loo_compare(loo(loyn_brm3b, moment_match = TRUE),
            loo(loyn_brm3e)
            )
loo_compare(loo(loyn_brm3c, moment_match = TRUE),
            loo(loyn_brm3e)
            )
loo_compare(loo(loyn_brm3d),
            loo(loyn_brm3e)
            )
```
...
bayes_factor(loyn_brm3a,
            loyn_brm3e)
#OR
bayes_factor(loyn_brm3e,
            loyn_brm3a)

bayes_factor(loyn_brm3b,
             loyn_brm3e)
#OR
bayes_factor(loyn_brm3e,
             loyn_brm3b)

bayes_factor(loyn_brm3c,
             loyn_brm3e)
#OR
bayes_factor(loyn_brm3e,
             loyn_brm3c)

bayes_factor(loyn_brm3d,
             loyn_brm3e)
#OR
bayes_factor(loyn_brm3e,
             loyn_brm3d)
...
## Prediction data (newdata)

::: {.panel-tabset}

### tidyverse (expand)

Range of AREA crossed with unique fGRAZE levels.
...
newdata <- loyn |>
    expand(
        ...
        fGRAZE = unique(fGRAZE)
    )
newdata |> head()
newdata |>
  group_by(fGRAZE) |>
  summarise_draws(range)
```

Different range of AREA crossed with unique fGRAZE levels.
...
#| cache: false
newdata <- loyn |>
    modelr::data_grid(AREA = modelr::seq_range(AREA, 10), fGRAZE)
newdata |> head()
newdata |>
  group_by(fGRAZE) |>
  summarise_draws(range)
...
newdata <- loyn |>
  group_by(fGRAZE) |>
  modelr::data_grid(AREA = modelr::seq_range(AREA, 10))
newdata |> head()
newdata |>
  group_by(fGRAZE) |>
  summarise_draws(range)
...
newdata <-
    marginaleffects::datagrid(
      newdata = loyn,
      AREA = function(x) modelr::seq_range(x, n = 100),
      fGRAZE = unique
    )
newdata |> head()
newdata |>
...
## or with the model instead of the raw data
newdata <-
    marginaleffects::datagrid(
      model = loyn_brm3b,
      AREA = function(x) modelr::seq_range(x, n = 100),
...
newdata <- emmeans(loyn_brm2, ~AREA|fGRAZE, at = loyn_list, type='response') |>
    as.data.frame()
head(newdata)
...
spaghetti = emmeans(loyn_brm2, ~AREA|fGRAZE, at = loyn_list, type = 'response') |>
  gather_emmeans_draws() |> mutate(Fit = exp(.value))
wch <- sample(1:max(spaghetti$.draw), 100,replace = FALSE)
spaghetti <- spaghetti |> filter(.draw %in% wch)
ggplot(newdata) +
...
newdata <- emmeans(loyn_brm3b, ~AREA|fGRAZE, at = loyn_list, type='response') |>
    as.data.frame()
head(newdata)
...
spaghetti = emmeans(loyn_brm3b, ~AREA|fGRAZE, at = loyn_list, type = 'response') |>
  gather_emmeans_draws() |> mutate(Fit = exp(.value))
wch <- sample(1:max(spaghetti$.draw), 100,replace = FALSE)
spaghetti <- spaghetti |> filter(.draw %in% wch)
ggplot(newdata) +
