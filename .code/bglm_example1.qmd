...
OR with more flexible caption and label
...
Yield of grass conditional on fertiliser concentration.
...
\beta_0 &\sim{} \mathcal{N}(164, 90)\\
\beta_1 &\sim{} \mathcal{N}(0, 90)\\
\sigma &\sim{} \mathcal{t}(3,0,90)\\
\end{align}
$$
...
(such as most Bayesian models), it is a good idea to cache the quarto
or Rmarkdown chunks in which the routine is performed. That way, the
...
- for effects parameters on scaled data, an argument could be made for
  ...
  (e.g. `normal(0,1)`), although some prefer a t distribution with 3
...
- for unscaled data, the above priors can be scaled by using the
  ...
  (e.g. `student_t(3,0,sd(y))`, or `student_t(3,0,sd(y)/sd(x))`)
...
- $\beta_1$: normal centred at 0 with a standard deviation of 65
...
it is a good idea to explore what they are. Firstly, out of curiosity,
it might be interesting to see what it has chosen. However, more
importantly, we need to be able to document what the priors were and
the `brms` development team make it very clear that there is no
guarantee that the default priors will remain the same into the
future.
...
- for the intercept, it is using a student t (flatter normal) prior
  with a mean of 161.5 and a standard deviation of 90.4. These mean
  and standard deviation values are based on the median and median
  absolute deviation of the response variable (rounded).

**Note, all priors are on the link scale and on scaled (e.g.
standardized to a mean of 0 and standard deviation of 1) predictiors**
...
standard deviation would be `2.5 *
mad(fert$YIELD)/mad(fert$FERTILIZER)` = `2.44` (which I will round to
2.5). However, if we scale the predictor (which we should), then we
are essentially dividing the `mad` or `sd` of the response by 1 (since
the standard deviation of a scaled vector is 1). Hence, we the
standard devation for the prior could just be `mad(fert$YIELD)`.
...
fert_brm0 |> ggpredict() |> plot(show_data=TRUE)
```

#### ggemmeans

```{r fitModel2e2, results='markdown', eval=TRUE, mhidden=TRUE}
fert_brm0 |> ggemmeans(~FERTILIZER) |> plot(show_data=TRUE)
```

#### conditional_effects

```{r fitModel2e3, results='markdown', eval=TRUE, mhidden=TRUE}
fert_brm0 |> conditional_effects() |>  plot(points=TRUE)
```
...
```{r fitModel2e4, results='markdown', eval=TRUE, mhidden=TRUE}
fert_brm0 |>
...
- $\beta_0$: normal centred at 164 with a standard deviation of 90
  - mean of 164: since `median(fert$YIELD)`
  - sd of 90: since `mad(fert$YIELD)`
- $\beta_1$: normal centred at 0 with a standard deviation of 90
  - sd of 90: since `mad(fert$YIELD)/1`
- $\sigma$: half-t centred at 0 with a standard deviation of 90 OR
  - sd pf 90: since `mad(fert$YIELD)`
- $\sigma$: gamma with shape parameters of 2 and 0.1
...
#| echo: true
#| cache: false
xx <- rnorm(n = 1000, mean = 164, sd = 60)
ggplot(fert, aes(y = YIELD)) +
  geom_boxplot(aes(x = 1)) +
  geom_boxplot(data = data.frame(x = 2, y = xx),
    aes(x = x, y = y))

ggplot(fert, aes(x = YIELD)) +
    geom_boxplot(aes(color = "Observed")) +
    geom_density(aes(y = after_stat(scaled), color = "Observed")) +
    geom_density(
        data = data.frame(x = xx),
        aes(x = x, y = after_stat(scaled), color = "Prior")
    )

ggplot(fert, aes(y = YIELD, x = scale(FERTILIZER))) +
  geom_point() +
  geom_smooth(method = "lm")
...
```{r fitModel2i4, results='markdown', eval=TRUE, mhidden=TRUE}
fert_brm1 |>
  marginaleffects::plot_predictions(condition = "FERTILIZER", points = 1)
```
...
fert_brm2 <- update(fert_brm1, sample_prior = 'yes', seed = 123, refresh = 0)
#OR
...
```{r fitModel2j2, results='markdown', eval=TRUE, cache=FALSE, echo = FALSE, mhidden=TRUE}
...
```{r fitModel2k4, results='markdown', eval=TRUE, mhidden=TRUE}
fert_brm2 |>
  marginaleffects::plot_predictions(condition = "FERTILIZER", points = 1)
```
...
fert_brm2 |> get_variables()
## fert_brm2 |> hypothesis('Intercept=0', class='b') |> plot
fert_brm2 |> hypothesis('scaleFERTILIZER = 0') |> plot()
## fert_brm2 |> hypothesis('scaleFERTILIZERscaleEQFALSE=0') |> plot
fert_brm2 |> hypothesis('sigma = 0', class = '') |> plot()
```
...
fert_brm2 |> get_variables()
fert_brm2 |> SUYR_prior_and_posterior()
```
...
fert_brm2 |> mcmc_plot(type = 'combo')
fert_brm2 |> mcmc_plot(type = 'trace')
fert_brm2 |> mcmc_plot(type = 'dens_overlay')
fert_brm2 |> mcmc_plot(type = 'nuts_treedepth')
fert_brm2 |> mcmc_plot(type = 'nuts_divergence')
fert_brm2 |> mcmc_plot(type = 'nuts_stepsize')
fert_brm2 |> mcmc_plot(type = 'nuts_energy')
```
...
fert_brm2 |> mcmc_plot(type = 'acf_bar')
```
...
fert_brm2 |> mcmc_plot(type = 'rhat_hist')
```
...
fert_brm2 |> mcmc_plot(type = 'neff_hist')
```
...
fert_brm2 |> mcmc_plot(type = 'combo')
fert_brm2 |> mcmc_plot(type = 'violin')
```
</details>
...
stan_trace(fert_brm2$fit)
fert_brm2$fit |> stan_trace()
fert_brm2$fit |> stan_trace(inc_warmup = TRUE)
```
...
fert_brm2$fit |> stan_ess()
```

  Ratios all very high.

```{r modelValidation2k, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
fert_brm2$fit |> stan_dens(separate_chains = TRUE)
```

```{r modelValidation2k_new, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
fert_brm2$fit |> check_hmc_diagnostics()
```
...
fert.ggs <- fert_brm2$fit |> ggs(inc_warmup = TRUE, burnin = TRUE)  # does not seem to ignore burnin?
fert.ggs |> ggs_traceplot()
fert.ggs <- fert_brm2$fit |> ggs(inc_warmup = FALSE, burnin = TRUE)  # does not seem to ignore burnin?
fert.ggs |> ggs_traceplot()
```
...
fert.ggs |> ggs_effective(version_effective = "BDA3")
```
...
fert_brm2 |> pp_check( type='dens_overlay', ndraws=100)
```
The model draws appear to be consistent with the observed data.
...
fert_brm2 |> pp_check(type='error_scatter_avg')
```
...
fert_brm2 |> pp_check(x='FERTILIZER', type='error_scatter_avg_vs_x')
```
...
fert_brm2 |> pp_check(x='FERTILIZER', type='intervals')
```

- ribbon: this is just an alternative way of expressing the above plot.

```{r modelValidation5f, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=6, fig.height=4}
pp_check(fert_brm2, x='FERTILIZER',type='ribbon')
```
...
fert.resids <- make_brms_dharma_res(fert_brm2, integerResponse = FALSE)
...
The following is the old and (now incorrect) way.

```{r modelValidation6a, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
preds <- fert_brm2 |> posterior_predict(ndraws=250,  summary=FALSE)
fert.resids <- createDHARMa(simulatedResponse = t(preds),
                            observedResponse = fert$YIELD,
                            fittedPredictedResponse = apply(preds, 2, median),
...
fert_brm2 |> conditional_effects()
fert_brm2 |> conditional_effects() |> plot(points = TRUE)
fert_brm2 |>
...
fert_brm2 |> ggpredict() |> plot(show_data=TRUE)
```

### ggemmeans

```{r partialPlot2b, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
fert_brm2 |> ggemmeans(~FERTILIZER) |> plot(show_data=TRUE)
```
...
- in the initial block of information, we are reminded on the formula as well as
  ...
  is `r nrow(as.matrix(fert_brm2))`.
- the estimated intercept (expected yield when fertilizer concentration is 0) is
  `r round(fert.sum$fixed[1,1],2)`.  This is the mean of the posterior distribution
  for this parameter.  Since we scaled the predictor, the intercept represents the expected
  mean of the response for the average fertilizer concentration.
- the estimated slope (rate at which yield changes per 1 unit change in
  scaled fertilizer concentration), is `r round(fert.sum$fixed[2,1],2)` (mean) with a
  standard error of `r round(fert.sum$fixed[2,2],2)`.  So this is the expected change in yeild for a moderate (1 standard deviation) change in fertiliser concentration.
  The 95% credibility intervals indicate that we are 95% confident that the slope is between
...
We can also express the slope as if we had not scaled the fertiliser predictor.

```{r summariseModel2d2a, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
...
##fert_brm2$data
##attr(fert_brm2$data[, 2], "scaled:scale")
fert_sigma <- sd(fert$FERTILIZER)
# OR
fert_sigma <- attr(fert_brm2$data[, "scale(FERTILIZER)"], "scaled:scale")
fert_brm2$fit |>
    tidy_draws() |>
    dplyr::select(matches('^b_.*|^sigma'))  |>
    ...
    summarise_draws(median,
                    ...
                    rhat, length, ess_bulk, ess_tail)
## for just the slope...
fert_brm2 |> emtrends(~ 1, var = "FERTILIZER")
```
...
```{r summariseModel2azz, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
fert_brm2 |>
  emtrends(~1, var = "FERTILIZER")
```

### model_parameters

```{r summariseModel2aa, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
fert_brm2 |>
...
fert.tidy <- tidyMCMC(fert_brm2$fit, estimate.method='median',  conf.int=TRUE,  conf.method='HPDinterval',  rhat=TRUE, ess=TRUE)
```
...
- the estimated intercept (expected yield when fertilizer concentration is 0) is
  ...
  mean of the response for the average fertilizer concentration.
- the estimated slope (rate at which yield changes per 1 unit change in
  scaled fertilizer concentration), is `r round(as.numeric(fert.gather[2,2]),2)` (mean) or (median).
  The 95% credibility intervals indicate that we are 95% confident that the slope is between
  `r round(as.numeric(fert.gather[2,3]),2)` and `r round(as.numeric(fert.gather[2,4]),2)` - e.g. there is a significant positive trend. So this is the expected change in yeild for a moderate (1 standard deviation) change in fertiliser concentration.
- sigma is estimated to be `r round(as.numeric(fert.gather[3,2]),2)`
...
fert_brm2 |> spread_draws(b_Intercept, b_scaleFERTILIZER, sigma)
# OR via regex
fert_brm2 |> spread_draws(`b_.*|sigma`,  regex=TRUE)
```

### bayesplot

```{r summariseModel2j, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
fert_brm2 |> mcmc_plot(type='intervals')
```
...
fert_brm2 |> posterior_samples() |> as_tibble()
```
...
fert_brm2 |> bayes_R2(summary=FALSE) |> median_hdci()
```
...
mcmcpvalue(as.matrix(fert_brm2)[, c("b_scaleFERTILIZER")])
```
...
```{r summariseModel2az, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
fert_brm2 |>
  avg_slopes()
```
...
| `marginaleffects` | 'predictions`         | Estimated marginal effects                                                                                 | `median_hdci()`                |
...
```{r predictions2baa, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
fert_brm2 |> predict(newdata = newdata)
```
...
```{r predictions2ba1, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
...
```{r predictions2cd, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
fert_brm2 |> marginaleffects::predictions(newdata)
dg <- datagrid(model = fert_brm2, FERTILIZER = 110)
fert_brm2 |> marginaleffects::predictions(newdata = dg)     #conditional
fert_brm2 |> marginaleffects::avg_predictions(newdata = dg) #marginal
```
...
fert_brm2 |> tidybayes::predicted_draws(newdata) |> median_hdci()
## or
newdata |> tidybayes::add_predicted_draws(fert_brm2) |> median_hdci()
## or
fert_brm2 |> posterior_predict(newdata=newdata) |>
    as.mcmc() |>
  tidyMCMC(estimate.method='median', conf.int=TRUE, conf.method='HPDinterval')
```
...
fert_brm2 |> epred_draws(newdata) |> median_hdci()
## or
newdata |> add_epred_draws(fert_brm2) |> median_hdci()
## or
fert_brm2 |> posterior_epred(newdata=newdata) |>
    as.mcmc() |>
  tidyMCMC(estimate.method='median', conf.int=TRUE, conf.method='HPDinterval')
```
...
fert_brm2 |> linpred_draws(newdata) |> median_hdci()
## or
newdata |> add_linpred_draws(fert_brm2) |> median_hdci()
## or
fert_brm2 |> posterior_linpred(newdata=newdata) |>
    as.mcmc() |>
  tidyMCMC(estimate.method='median', conf.int=TRUE, conf.method='HPDinterval')
```
...
Xmat = model.matrix(~FERTILIZER, data=newdata)
### get the posterior draws for the linear predictor
coefs <- fert_brm2 |> posterior_samples(pars=c('b_Intercept','b_scaleFERTILIZER')) |> as.matrix()
fit <- coefs %*% t(Xmat)
fit |> median_hdci() |> bind_cols(newdata)
```
...
fert_brm2 |> hypothesis('scaleFERTILIZER > 0')
fert_brm2 |> hypothesis('scaleFERTILIZER > 0') |> plot (ignore_prior= TRUE)
fert_brm2 |> hypothesis('scaleFERTILIZER > 0') |> _[['samples']] |> median_hdci()
```
...
fert_brm2 |> hypothesis('scaleFERTILIZER > 0.9')
fert_brm2 |>
...
```{r Probability2ba1, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5,paged.print=FALSE}
fert_brm2 |>
    tidy_draws() |>
...
fert_brm2 |>
    tidy_draws() |>
    ggplot(aes(x=b_scaleFERTILIZER)) +
...
fert_brm2 |> get_variables()
fert_brm2 |>
  as_draws_df(variable = "b_scaleFERTILIZER") |>
  summarise_draws(median,
    HDInterval::hdi,
    P = ~ mean(.x > 0)
  )
fert_brm2 |>
  as_draws_df(variable = "^b_.*FERT.*", regex = TRUE) |>
  summarise_draws(median,
    HDInterval::hdi,
    P = ~ mean(.x > 0)
  )
## What if we wanted it per unit on the original scale
fert_brm2 |>
  as_draws_df(variable = "^b_.*FERT.*", regex = TRUE) |>
  mutate(FERTILIZER = b_scaleFERTILIZER / fert_sigma) |>
  summarise_draws(median,
...
```{r Probability2ba2, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5,paged.print=FALSE}
fert_brm2 |>
...
bayestestR::equivalence_test(fert_brm2,
                             parameters = "FERTILIZER",
                             ## range = c(-0.08, 0.08),
                             range = c(-6.4, 6.4),
                             ci_method = "HDI"
...
fert_brm2 |> emmeans(~FERTILIZER,  at=newdata) |>
    regrid(transform = "log") |>
    pairs() |>
    regrid() |>
    tidy_draws() |>
...
library(modelr)
fert.grid <- fert |> data_grid(FERTILIZER = seq_range(FERTILIZER, n = 100))
...
## spaghetti plot
newdata = emmeans(fert_brm2, ~FERTILIZER, at = fert.grid) |>
    gather_emmeans_draws()
newdata |> head()
ggplot(newdata,  aes(y = .value,  x = FERTILIZER)) +
...
fert.list = with(fert, list(FERTILIZER = seq(min(FERTILIZER), max(FERTILIZER), len=100)))
newdata = emmeans(fert_brm2, ~FERTILIZER, at=fert.list) |> as.data.frame()
head(newdata)
...
## spaghetti plot
newdata = emmeans(fert_brm2, ~FERTILIZER, at=fert.list) |>
    gather_emmeans_draws()
newdata |> head()
ggplot(newdata,  aes(y=.value,  x=FERTILIZER)) +
    geom_line(aes(group=.draw),  colour = 'orange', alpha=0.01) +
