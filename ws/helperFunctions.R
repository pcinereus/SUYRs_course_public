...
SUYR_prior_and_posterior <- function(mod) {
    ...
    f <- no.offset(f)
    ## covariances
    wch.cov <- rhs[grep("ar\\(|arma\\(|ma\\(", rhs)]
    if (length(wch.cov)>0) f <- update(f, paste("~ . -",wch.cov))
    ...
    mod.pp <- mod %>%
        ...
        mutate(Type = ifelse(str_detect(key, 'prior'), 'Prior', 'Posterior'),
               ...
               ## deal with awkward spline names
               Parameter = ifelse(Type == 'Posterior' & str_detect(Parameter, "bs_.*"),
                                   str_remove(Parameter, "_.*"),
                                  Parameter),
      ...
      mutate(value = ifelse(Type == "Posterior" & Parameter == "shape", 1/value, value)) %>%
        suppressWarnings()
